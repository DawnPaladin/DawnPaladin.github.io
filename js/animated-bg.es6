...
const animatedBg = function() {
	...
	exports.run = function() {
		...
		function drawConnectingLine(shape, leaf) {
			...
			const grabber = new createjs.Shape();
			grabber.graphics.setStrokeStyle(2)
				.beginStroke(treeColor)
		...
		function tick(event) {
			...
			if (!createjs.Ticker.getPaused()) {
				...
				if (treeTip.x < 10 || treeTip.x > 600) { // reset if tree gets far outside its area
				...
				if (treeTip.x < 300) { // gentle nudge if it's close
